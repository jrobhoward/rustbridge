//! json-plugin - A rustbridge plugin for JSON validation and pretty-printing
...
mod tests {
    ...
    async fn validate___valid_json___returns_true() {
        ...
        let response = plugin
            .handle_request(&ctx, "validate", &request)
            .await
            .unwrap();
    ...
    async fn prettify___compact_json___returns_formatted() {
        ...
        let response = plugin
            .handle_request(&ctx, "prettify", &request)
            .await
            .unwrap();

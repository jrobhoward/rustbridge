...
/**
 * Verify that logging works correctly after reload (JNI implementation).
...
class ReloadLoggingVerificationTest {
    ...
    @Test
    @DisplayName("Logging works after reload with new callback (JNI)")
    void testLoggingWorksAfterReload() throws PluginException, InterruptedException {
        ...
        System.out.println("=== First Load (JNI) ===");
        Plugin plugin1 = JniPluginLoader.load(PLUGIN_PATH.toString(), PluginConfig.defaults(), callback1);
        ...
        System.out.println("\n=== Second Load (Reload) (JNI) ===");
        Plugin plugin2 = JniPluginLoader.load(PLUGIN_PATH.toString(), PluginConfig.defaults(), callback2);
    ...
    @Test
    @DisplayName("Log level can be set after reload (JNI)")

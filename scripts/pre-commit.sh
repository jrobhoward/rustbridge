...
if [ "$SMART_MODE" = true ]; then
    ...
    if [ -z "$ALL_CHANGES" ]; then
        ...
        if [ -n "$SCRIPT_CHANGES" ] || [ -n "$CONFIG_CHANGES" ]; then
            ...
            # Special case: Rust FFI changes should trigger Java/C#/Python/Go tests
        ...
        echo ""
        print_info "Test plan: Rust=$RUN_RUST_TESTS, Java=$RUN_JAVA_TESTS, C#=$RUN_CSHARP_TESTS, Python=$RUN_PYTHON_TESTS, Go=$RUN_GO_TESTS, Fmt=$RUN_RUST_FMT, Deny=$RUN_CARGO_DENY"
        echo ""
    fi
fi
